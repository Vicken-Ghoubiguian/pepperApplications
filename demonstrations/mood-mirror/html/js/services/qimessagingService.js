define(['./module'], function (services) {
    'use strict';
    services.factory('$qi', function($q, $rootScope, $window, $timeout, $location) {
        var sessionHost = undefined;
        var robotIp = window.getRobotIp();
        if (robotIp) {
            sessionHost = robotIp + ":80"
        }
        var qim = new QiSession(sessionHost);
        ...
        var watchEvent = function (event, handle){
            call(function(ALMemory) {
                ALMemory.getData(event).done(handle);
                ALMemory.subscriber(event).then(function (subscriber){
                    subscriber.signal.connect(handle);
                }, onQimError);
            });
        };
        return {
